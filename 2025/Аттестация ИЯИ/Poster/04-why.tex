...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			\raggedright
			\adjustbox{valign=t}{\begin{minipage}[t]{\linewidth}
				\centering{\textbf{Легкие частицы}}\\
	...
	\end{columns}
	\vspace{2em}
	При разработке структуры, удовлетворяющей требованиям, предъявляемым к частицам с различным зарядом, важно создать перестраиваемую структуру без внесения конструктивных изменений. Мы назвали такой подход -- проектированием \textit{дуальной} структуры.	
\end{frame}
%------------------------------------------------
\begin{frame}
...
\begin{frame}
	...
	\end{column}
\end{columns}
\footnotetext[1]{Kolokolchikov, S.D., Senichev, Y.V. Magneto-Optical Structure of the NICA Collider with High Transition Energy. Phys. Atom. Nuclei 84, 1734–1742 (2021). https://doi.org/10.1134/S1063778821100185}
\end{frame}
%------------------------------------------------
\begin{frame}
...
\begin{frame}
	...
	\begin{equation}
		...
		M_{\text{pk}} =\frac{1}{2\left(f_{\max}+f_{\min}\right) \eta_{\textrm{pk}} T_{\textrm{pk}} \frac{\Delta p}{p}}; \quad 
		M_{\text{kp}} =\frac{1}{2\left(f_{\max}-f_{\min}\right) \eta_{\textrm{kp}} T_{\textrm{kp}} \frac{\Delta p}{p}}.
	\end{equation}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\adjustbox{valign=t}{\begin{minipage}[t]{\linewidth}
				...
				\begin{enumerate}
					\item $\eta\rightarrow\frac{1}{2\left(f_{\text{max}}+f_{\text{min}}\right)T_{\text{pk}}\frac{\Delta p}{p}}$, Schottky-спектр пучка становится сплошным и $M_{\text{pk}}\rightarrow1$;
...
\begin{frame}
	...
	\begin{columns}
		...
		\end{column}
	\end{columns}
\footnotetext[2]{(to be published) Kolokolchikov, S.D., et al. Features of dual-purpose structure for heavy ion and light particles, Nucl.Sci. and Tech.}
\end{frame}
%------------------------------------------------
%------------------------SECOND------------------------
%------------------------------------------------
...
\footnotetext[3]{Kolokolchikov, S.D., Senichev, Y.V. \& Kalinin, V.A. Transition Energy Crossing in Harmonic RF at Proton Synchrotron U-70. Phys. Atom. Nuclei 87, 1355–1362 (2024). https://doi.org/10.1134/S106377882410020X}
\end{frame}
%------------------------------------------------
\begin{frame}
	...
	\centering
	\includegraphics[width=0.4\columnwidth]{3_tunes_vs_g_tr.png}
	\includegraphics[width=0.4\linewidth]{3_Jump_NICA}
...
\footnotetext[4]{Kolokolchikov, S., Senichev, Y., Aksentev, A. et al. Longitudinal Dynamic in NICA Barrier Bucket RF System at Transition Energy Including Impedances in BLonD. Phys. Part. Nuclei Lett. 21, 419–424 (2024). https://doi.org/10.1134/S1547477124700389}
\end{frame}
%------------------------------------------------
\begin{frame}
...
\footnotetext[5]{Kolokolchikov, S.D., Aksentev, A.E., Mel’nikov, A.A. et al. Transition Energy Crossing in NICA Collider of Polarized Proton Beam in Harmonic and Barrier RF. Phys. Atom. Nuclei 87, 1449–1454 (2024). https://doi.org/10.1134/S1063778824100211}
\end{frame}
%------------------------------------------------
%------------------------THIRD------------------------
%------------------------------------------------
...
\begin{frame}
	...
	\end{equation}
	\centering
	\includegraphics[width=0.4\columnwidth]{4_total_length}
\footnotetext[1]{Работа: [название работы], [автор(ы)]}
\end{frame}
%------------------------------------------------
\begin{frame}
...
\begin{frame}
	\frametitle{Модернизация Nuclotron 16-ти периодическая структура}
		...
		\end{column}
	\end{columns}
\footnotetext[6]{(to be published) Kolokolchikov, S.D., et al. Quasi-frozen spin for both deuteron and proton beam at periodic EDM storage ring lattice, Nucl.Sci. and Tech.}
\end{frame}
%------------------------------------------------
\begin{frame}
	\frametitle{NICA bypass}
	\centering
	\includegraphics[width=0.8\columnwidth]{4_NICA_bypass.png}
\footnotetext[7]{Kolokolchikov, S., Aksentyev, A., Melnikov, A. et al. Designing Bypass Channels in NICA Accelerator Complex for Polarized Beam Experiments for EDM Search. Phys. Atom. Nuclei 86, 2423–2428 (2023). https://doi.org/10.1134/S1063778823110248}
\end{frame}
%------------------------------------------------