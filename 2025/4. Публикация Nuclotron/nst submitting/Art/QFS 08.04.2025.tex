...
\par This paper explores a versatile structure characterized by spatially segregated magnetic and electrostatic fields, uniquely designed to facilitate the study of EDM for both deuterons and protons. A notable feature of this structure is its ability to support experiments for both types of particles, thereby enhancing its applicability and efficiency. Special emphasis is placed on the utilization of Wien filters or electrostatic deflectors with kicker to compensate for the magnetic dipole moment (MDM) and establish conditions conducive to a quasi-frozen spin (QFS). The analysis elucidates how such a structure can be adapted for facilities not originally intended for such experiments and explores its potential for advancing research at higher energy levels.
...
to the EDM value $d$ and spin $s$ of a particle: $d = \frac{\eta g}{2mc}s$, $G=\frac{g-2}{2}$ -- 
...
\par An alternative approach is the quasi-frozen spin concept, based on the principle of sequential spin compensation \cite{QFS}. This leads to the spin oscillations relative to the momentum at one period and as a result at full ring.

\subsection{Relative spin motion at QFS}    

\par Let us introduce the concept of QFS by examining orbital and spin rotation in a generalized framework. This approach is based on the dynamics governed by the Lorentz force for orbital motion and T-BMT equation for spin precession. It is essential to ensure first QFS condition that the orbital trajectory remains unaltered, maintaining a closed orbit configuration.
...
\par In a magnetic field, according to eq.\ref{eq:spin-tine} the direction of rotation depends solely on the sign of the anomalous magnetic moment and is independent of energy. However, the magnitude of the relative rotation is influenced by both the energy and absolute value of anomalous magnetic moment. For protons this effect is so important as spin rotation per arc should be limited $\gamma G \cdot \Phi_{p}^{\text{arc}} < \pi/2$ to save an ability for EDM accumulation for longitudinally polarized beam. And usage of periodic structures can save the idea of measuring proton EDM with QFS lattice, should be at least $N=8$ and greater. 
\par More precisely, the EDM measurement slightly deviates from the frozen spin condition, in the first order decreases as given by the factor \cite{consider}
...
\noindent where $\Phi_s^{\textrm{arc}}$ represents angle associated with the MDM-effect at arc in period. For various particle and structure periodicity  it states at Table \ref{tab:edm}. Maximum periodicity considered as $N=16$, due to lattice design difficulties. Shown that for deuterons QFS lattice close to FS. For protons, 16-periodic structure can provide a real opportunity to practice the methodology.
...
\par In electrostatic fields, the behavior of protons and deuterons is further complicated by their distinct interactions with the field. For protons, the direction of spin rotation can depend on the energy, particularly around the "magic energy" where $\gamma_{\text{mag}} = \sqrt{\cfrac{G_{\text{p}}+1}{G_{\text{p}}}} = 1.248$ ($233$ MeV). At any way the influence on proton is much lower than for deuterons as shown at Fig. \ref{fig:spin-tune}. For deuterons, however, there is no equivalent "magic energy," and their spin rotation behavior remains consistent across energy levels.\\
...
\begin{thebibliography} {99}
	\bibitem{EDM} M.Pospelov, A.Ritz, Electric dipole moments as probes of new physics. Annals of Phys. \textbf{318}, 119-169 (2005). doi:10.1016/j.aop.2005.04.002
	...
	\bibitem{consider} Senichev, Y.V., Aksentyev, A.E., Kolokolchikov, S.D. et al. Consideration of an Adapted Nuclotron Structure for Searching for the Electric Dipole Moment of Light Nuclei. Phys. Atom. Nuclei \textbf{86}, 2434–2438 (2023). doi:10.1134/S1063778823110418
